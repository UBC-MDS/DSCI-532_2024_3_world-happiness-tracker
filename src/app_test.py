...
def update_map(country1, year):
    ...
    if country1:
        ...
        layer_fade = px.choropleth(map_df,locations="Country", color="show",locationmode="country names",# showlegend = False,
                                     color_discrete_sequence=["rgba(255,255,255,0.7)", "rgba(255,255,255,0)"])
        layer_fade_dict = layer_fade.data[0]
        layer_fade_dict["showlegend"] = False
        fig.add_trace(layer_fade_dict)
